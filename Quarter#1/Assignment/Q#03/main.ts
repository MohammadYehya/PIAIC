let Name: string = "Mohammad yehya hayati";
...
function toTitleCase(Name: string): string
{
    ...
    for (let i = 0; i < str.length; i++) {str[i] = str[i][0].toUpperCase() + str[i].slice(1, str[i].length);}

    //Method#2 - ForEach method
    str.forEach(word => ret += word[0].toUpperCase() + word.slice(1, word.length)+ " ");
