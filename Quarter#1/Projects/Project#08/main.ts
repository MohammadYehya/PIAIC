...
class Question
{
    ...
    constructor(q: string, ans: string[], correctidx: number[])
    {
        this.q = q;
        this.ans = ans;
...
for(let i = 0; i < Q.length; i++)
{
    ...
    let ans = Q[i].ans.filter((element) => {for(let j = 0; j < Q[i].correctidx.length; j++){if(element === Q[i].ans[Q[i].correctidx[j]])return element;}});
    console.log(`${ans} was/were the correct answer(s)!`);
    if(ans.find(element => element === marks.ans) != undefined) correct++;
}
