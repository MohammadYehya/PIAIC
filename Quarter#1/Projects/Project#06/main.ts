...
class Student
{
    ...
    balance !: number;
    paidfees : boolean = false;
    tuitionfees: number = 0;
    ...
    enroll(course: {name: string, price: number}[])
    {
        if(this.CoursesEnrolled.findIndex((element, idx) => element === course[0].name) === -1) {this.CoursesEnrolled.push(course[0].name); this.tuitionfees += course[0].price; this.paidfees = false;}
        else console.log(`Already enrolled in ${course[0].name}!`);
    }
    ViewBalance()
    {
    ...
    PayTuitionFees()
    {
        ...
        else if (this.paidfees) console.log('Fees already paid!');
        else console.log(`Insufficient balance to pay tuition fees ${this.tuitionfees}!\n Current Balance: ${this.balance} `);
    }
    ShowStatus()
    {
...
while(true)
{
    ...
    switch(choice.val)
    {
        ...
        case 2:
            ...
            Std[0].enroll(OfferedCourses.filter((element) => {if(element.name === c.val) return element;}));
            await wait();
            break;
        case 3:
            Std[0].ViewBalance();
